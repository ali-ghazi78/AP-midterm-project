...
DLS::Node::Node(const std::vector<int> &initial_state)
{
...
void DLS::Node::set_parent_number()
{
    Node *n = this;
    number_of_parent = n->parent->number_of_parent;
    number_of_parent++;
}
void DLS::move_zero(std::vector<int> &vec1, int x, int y, int loc)
{
...
bool DLS::check_if_is_answer(const std::vector<int> &v)
{
...
size_t DLS::show_path(DLS::Node *n)
{
    size_t steps = 0;
    while (n != nullptr)
    {
...
int DLS::search_for_answer(Node *cu_node)
{
    ...
    {
        std::cerr << "a problem is occured maybe depth is not enough" << DLS::Node::Node_no << std::endl;
        return 2;
    }
    current_node = cu_node;
...
std::string DLS::make_str(Node *n, const std::vector<int> &vec)
{
    std::string s = "";
    if (vec[0] == -1)
    {
...
bool DLS::my_find(const std::string &my_str, bool edit)
{
...
void DLS::disp_in_menu(const std::vector<int> &v, const std::vector<int> &v2)
{
    BFS b(v);
    b.disp_in_menu(v, v2);
}
